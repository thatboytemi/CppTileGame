...
namespace ANXTEM001{
    ...
    TileManager::TileManager(int numMoves, int dimension, int numTiles, int width, int height, unsigned char *image, std::string stateName, std::string summaryName): numMoves(numMoves), numTiles(numTiles), dimension(dimension), width(width), height(height){
        int rowCounter =0;
        ...
        writeToFile(tileWidth*dimension, tileHeight*dimension, stateName);
        currentMove++;
    ...
    void TileManager::writeToFile(int xdim, int ydim, std::string stateName){
        std::string filename = stateName+std::to_string(currentMove)+ ".pgm";
        std::ofstream outputFile;
        outputFile.open(filename, std::ios::binary);
        outputFile << "P5" <<  "\n" <<"#File generated by ANXTEM001" << "\n" << std::to_string(xdim) + " " + std::to_string(ydim) << "\n" << "255" <<"\n";
